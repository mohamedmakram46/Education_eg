...
• These analyses can be used to support educational decisions or improve performance in schools ''')
    st.header('Q7 What is the general distribution of student evaluations?')
    grade_counts = df["Student classification"].value_counts().sort_index()
    st.plotly_chart(px.pie(data_frame= df, names= 'Student classification'))
    st.write(''' #### This analysis shows that most of the values fill into the (Pass) or (Good) category. This means that most students are performing at an average level and there is an opportunity to improve performance. ''')
    ...
    Average_Score = st.sidebar.selectbox('Average_Score', df['Average Score'].unique())
    Student_classification = st.sidebar.selectbox('Student_classification', df['Student classification'].unique())
    df2 = df[(df['father degree']== father_degree) & (df['education type'] == education_type) &(df['student year']== student_year) &(df['mother degree']== mother_degree) & (df['Average Score'] == Average_Score) & (df['Student classification'] == Student_classification)]
